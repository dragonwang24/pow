...
public class PaginationInterceptor implements Interceptor {
	...
	private Logger logger = LoggerService.getTraceLogger(this.getClass());
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof Map<?, ?>) {
			...
			if(param.containsKey(Page.KEY)){
				Object objPage = param.get(Page.KEY);
				...
				if (objPage != null && objPage instanceof Page) {
					...
					// 给当前的page参数对象设置总记录数
					this.setTotalRecord(param, mappedStatement, connection);
					// 获取分页Sql语句
	...
	private String getDB2PageSql(Page page, StringBuffer sqlBuffer) {
		...
		if(page.getSortName() != null){
			sqlBuffer.append(" order by " + page.getSortName());
			if(page.getSortOrder() != null){
				sqlBuffer.append(" " + page.getSortOrder());
			} 
		}
		sqlBuffer.insert(0, "select u.*, rownumber() over() as rn from (").append(") u ");
		sqlBuffer.insert(0, "select * from (").append(")as a where a.rn between ").append(start).append(" and ").append(end);
...
//		int start = ((page.getPageNumber()-1)*page.getPageSize())+1;
//		int offset = start * page.getPageSize();
	...
	private String getOraclePageSql(Page page, StringBuffer sqlBuffer) {
		...
		// int offset = (page.getStart()) * page.getPageSize() + 1;
		//int offset = page.getStart() + 1;   modify by dengwq
		int start = ((page.getPageNumber()-1)*page.getPageSize())+1;
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecord(Map<String, Object> param, MappedStatement mappedStatement,Connection connection) {
		Page page = (Page)param.get(Page.KEY);
		...
		if(resultMaps.size()>0){
			...
			for(ResultMapping rm:resultMappingList){
				if(rm.getProperty().equals(page.getSortName())){
					page.setSortName(rm.getColumn());
				}
			}
		}
		...
		String countSql = CoreSQLParser.getCountSQL(sql);
		...
		ParameterHandler parameterHandler = new DefaultParameterHandler(
				mappedStatement, param, countBoundSql);
